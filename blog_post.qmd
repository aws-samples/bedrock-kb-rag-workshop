...
![](./img/ML-15729-blog_post.png){width=20%}
...
1. Set the vector index name as `sagemaker-readthedocs-io`, vector field name as `vector` dimensions as `1536`, and distance metric as `Euclidean`. **It is required that you set these parameters exactly as mentioned here because the Bedrock Knowledge Base Agent is going to use these same values**.
    ...
    ![SageMaker Knowledge Base Vector Index Parameters](img/ML-15729-aoss1.jpg){#fig-aoss-collection-vector-index-parameters}

1. Once created the vector index is listed as part of the collection.

    ![SageMaker Knowledge Base Vector Index Created](img/ML-15729-aoss2.jpg){#fig-aoss-collection-vector-index-created}
...
1. Create a new Bedrock agent, call it `sagemaker-qa` and use the `AmazonBedrockExecutionRoleForAgenta_QA` IAM role, this role is created automatically via CloudFormation.

    ![Provide agent details - agent name](img/ML-15729-agt2-s1-1.png){#fig-br-agt-create-step1-1}

    ![Provide agent details - IAM role](img/ML-15729-agt2-s1-2.png){#fig-br-agt-create-step1-2}

1. Provide the following as the instructions for the agent: `You are a Q&A agent that politely answers questions from a knowledge base`.

    ![Select model](img/ML-15729-agt3-s1.png){#fig-br-agt-select-model}
...
1. Select the `sagemaker-docs` knowledge base, in the knowledge base instructions for agent field entry `Answer questions about Amazon SageMaker based only on the information contained in the knowledge base`.
    ...
    ![Add knowledge base](img/ML-15729-agt3-s1.png){#fig-br-agt-add-kb}

1. Press the `Create Agent` button on the `Review and create` screen.
...
1. Once the agent is ready, we can ask questions to our agent using the Agent console.
    ...
    ![Agent console](img/ML-15729-agt7.png){#fig-br-agt-console}
